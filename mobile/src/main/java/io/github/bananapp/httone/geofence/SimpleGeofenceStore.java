...
public class SimpleGeofenceStore {
    ...
    private static final String KEY_IDS = "io.github.bananapp.httone.geofence.KEY_IDS";
    ...
    public void setGeofences(List<SimpleGeofence> geofences) {
        ...
        final HashSet<String> ids = new HashSet<String>(geofences.size());

        for (SimpleGeofence geofence : geofences) {

            ids.add(geofence.getId());
            setGeofence(geofence.getId(), geofence);
        }

        mPrefs.edit().putStringSet(KEY_IDS, ids).commit();
    }

    public List<SimpleGeofence> getGeofences() {

        final Set<String> ids = mPrefs.getStringSet(KEY_IDS, Collections.<String>emptySet());
        final ArrayList<SimpleGeofence> geofences = new ArrayList<SimpleGeofence>(ids.size());

        for (String id : ids) {

            geofences.add(getGeofence(id));
        }
